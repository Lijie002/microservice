...
import com.qixinmini.oauth.properties.CaptchaProperties;
...
public class CaptchaServiceImpl implements CaptchaService {
    ...
    public void checkCaptcha(String key,String code) {
        if (StringUtils.isBlank(code)) {
            throw new MicroserviceException("请输入验证码");
        }
        String value = (String)redisService.get(CaptchaConstant.CAPTCHA_KEY_PREFIX+key);
        if(StringUtils.isBlank(value)){
            throw new MicroserviceException("验证码已过期");
        }
        if(!StringUtils.equals(code,value)){
            throw new MicroserviceException("验证码不正确");
        }
    }
